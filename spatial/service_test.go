...
func (f *fakeSpaceIoT) Space(_ context.Context, id int64) (tuya.Space, error) {
	f.queried = id
...
func (f *fakeSpaceIoT) ListSpaces(_ context.Context, id int64, _ bool, _ tuya.Page) ([]int64, tuya.Page, error) {
	f.listed = append(f.listed, id)
	return []int64{insideRoom}, tuya.Page{}, nil
}

func (f *fakeSpaceIoT) SpaceResources(_ context.Context, id int64, _ bool, _ tuya.Page) ([]tuya.Resource, tuya.Page, error) {
	f.resourcesOf = id
	f.resourceCalls++
	if f.endlessPages {
...
func newSpaceDoor(t *testing.T, iot *fakeSpaceIoT) *Service {
	t.Helper()
	store := &fakeSpaceStore{space: Space{Owner: "owner-1", SpaceID: ownerSpace}}
	return NewService(iot, store)
}
...
func TestContainsDeviceScansTheWholeSubtree(t *testing.T) {
	...
	iot := &fakeSpaceIoT{pages: []resourcePage{{resources: []tuya.Resource{lobbyLight}}}}
	door := newSpaceDoor(t, iot)
...
func TestContainsDeviceAbsentIsFalseNotError(t *testing.T) {
	iot := &fakeSpaceIoT{pages: []resourcePage{{resources: []tuya.Resource{{ID: "dev-other", Type: tuya.SpaceResourceDevice}}}}}
	door := newSpaceDoor(t, iot)
